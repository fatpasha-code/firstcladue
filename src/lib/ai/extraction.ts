...
export async function runExtraction(rawText: string): Promise<ExtractedData> {
  ...
  try {
    ...
    if (isConnectionError(error)) {
      console.warn('[runExtraction] ECONNRESET — retrying once')
      try {
        return await attempt().then(async (message) => {
          const textBlock = message.content.find(b => b.type === 'text')
          const text = textBlock?.type === 'text' ? textBlock.text : ''
          const cleaned = text.trim().replace(/^```(?:json)?\n?/, '').replace(/\n?```$/, '')
          return ExtractedDataSchema.parse(JSON.parse(cleaned))
        })
      } catch (retryError) {
        console.error('[runExtraction] retry failed:', retryError)
        throw retryError
      }
    }
